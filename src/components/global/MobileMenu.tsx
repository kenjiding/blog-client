...
export default function MobileMenu() {
  ...
  return (
    <>
      ...
      <div
        className={`md:hidden absolute left-0 top-0 z-10 h-full w-full transform duration-[600ms] ease-[cubic-bezier(0.7,0,0,1)] dark:bg-zinc-900 bg-white ${
          navShow ? "translate-x-0 rounded-none" : "translate-x-full hidden"
        }`}
      >
        ...
        <nav className="flex flex-col mt-6 bg-black h-screen bg-opacity-80">
          {MenuData.map((link) => (
            <Link
              key={link.title}
              href={link.href}
              className="bg-black flex items-center gap-x-2 font-incognito font-semibold text-lg dark:shadow-line-dark shadow-line-light p-6 group"
              onClick={onToggleNav}
            >
              <link.icon
