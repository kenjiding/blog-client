class AnimatedClockCom extends HTMLElement {
  constructor() {
    super();
    ...
    * {
    ...
    section.clock div.hours,
    section.clock div.seconds {
      width: 1px;
      height: 1px;
      position: absolute;
      left: 50%;
    ...
    template.innerHTML = `
      <section class="clock">
        ...
        .map((i) => `<li><i style="color: #000;">${i}</i></li>`)
        .join("")}
        </ul>
        <h2>ophelia</h2>
  ...
  initializeClock() {
    ...
    const dateOutput = this.shadowRoot.querySelector(".date");
    ...
    const updateClock = () => {
      ...
      clock.querySelector(".hours").style.transform = `rotate(${deg.hours}deg)`;
      clock.querySelector(".minutes").style.transform = `rotate(${deg.minutes}deg)`;
      clock.querySelector(".seconds").style.transform = `rotate(${deg.seconds}deg)`;
      ...
      dateOutput.textContent = `${time.getDate()} / ${months[time.getMonth()]}`;
    };
...
// 定义 Web Component
customElements.define("animated-clock-com", AnimatedClockCom);
